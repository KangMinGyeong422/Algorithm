...
class DijkstraScreen(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
    
    def initUI(self):
        self.setWindowTitle('다익스트라 최단 경로')
        self.resize(1280, 720)
        
        # label
        label = QLabel('다익스트라 최단 경로 알고리즘', self)
        label.setAlignment(Qt.AlignCenter)
        font = label.font()
        font.setPointSize(30)
        label.setFont(font)
        
        des = QLabel('\n그래프는 입력이 복잡할 것 같아 교재에 등장하는 그래프를 입력했습니다.')
