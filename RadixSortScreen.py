...
class RadixSortScreen(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('기수 정렬')
    ...
    def sortData(self):
        ...
        # python에서 C코드 호출
        result = self.runCRadixSort(arraySize, arrayValue)

        # 결과 표시
        self.resultLabel.setText(f'{result}')

    def runCRadixSort(self, arraySize, arrayValue):
        # C 코드 실행 결과
        command = ['RadixSort.exe']
