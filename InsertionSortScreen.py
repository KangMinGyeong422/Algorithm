...
class InsertionSortScreen(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('삽입 정렬')
    ...
    def sortData(self):
        ...
        # python에서 C코드 호출
        result = self.runCInsertionSort(arraySize, arrayValue)

        # 결과 표시
        self.resultLabel.setText(f'{result}')

    def runCInsertionSort(self, arraySize, arrayValue):
        # C 코드 실행 결과
        command = ['InsertionSort.exe']
