...
class BubbleSortScreen(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('버블 정렬')
    ...
    def sortData(self):
        ...
        # python에서 C코드 호출
        result = self.runCBubbleSort(arraySize, arrayValue)

        # 결과 표시
        self.resultLabel.setText(f'{result}')

    def runCBubbleSort(self, arraySize, arrayValue):
        # C코드 실행 결과
        command = ['BubbleSort.exe']
