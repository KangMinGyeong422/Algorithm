...
class QuickSortScreen(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('퀵 정렬')
    ...
    def sortData(self):
        ...
        # python에서 C코드 호출
        result = self.runCQuickSort(arraySize, arrayValue)

        # 결과 표시
        self.resultLabel.setText(f'{result}')

    def runCQuickSort(self, arraySize, arrayValue):
        # C 코드 실행 결과
        command = ['QuickSort.exe']
